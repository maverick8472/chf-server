...
const {User, validateAuth, validateRegister, validateEdit, validateRole} = require('../models/user');
...
async function registerUser(req, res){
    ...
    if(user) {
        if(user.isDeleted == true){
            ...
            res.header('x-auth-token',token).send(_.pick(user, ['_id','username','age']));
...
async function loginUser(req, res){
    ...
    const token = user.generateAuthToken();
    res.header('x-auth-token',token).send(_.pick(user, ['_id','username','role']));
}
